...
export class DemographicBarComponent implements OnInit {
  ...
  public statusDisease = [
    ...
    { name: 'Not Active', disabled: false }
  ];
  public statusType = [
    { name: 'Select', disabled: false },
    { name: 'Progressive', disabled: false },
    { name: 'Not Progressive', disabled: false }
  ];
  ...
  constructor(private service: EhrService, private support: SupportService, private session: SessionService, private activityService: ActivityService, private auth: AuthenticationService, private msService: MsService, private appoService: AppointmentsService) {
    ...
    this.session.getParams().subscribe(params => {
      if (params['pom_id']) {
        ...
        this.session.getUser().subscribe(user => {
          ...
          this.appoService.fetchAppointments('1', this.pomId, this.activityService.moment().subtract(2, 'years').format('MM/DD/YYYY', 'en', true), this.activityService.moment().add(6, 'months').format('MM/DD/YYYY', 'en', true), this.userObject.provider_id).subscribe(appointments => {
        ...
        this.msService.getMSPatientData(this.pomId).subscribe(data => {
          ...
          this.activity.age_of_onset_last_updated_instant = this.activityService.moment(new Date(data['age_of_onset_last_updated_instant'])).format('MM/DD/YYYY', 'en', true);
          this.activity.ms_type_last_updated_instant = this.activityService.moment(new Date(data['ms_type_last_updated_instant'])).format('MM/DD/YYYY', 'en', true);
          this.activity.ms_status_1_last_updated_instant = this.activityService.moment(new Date(data['ms_status_1_last_updated_instant'])).format('MM/DD/YYYY', 'en', true);
          this.activity.ms_status_2_last_updated_instant = this.activityService.moment(new Date(data['ms_status_2_last_updated_instant'])).format('MM/DD/YYYY', 'en', true);
        ...
        this.service.getDemographicsM2(this.pomId).subscribe(data => {
          if (data['EPIC']) {
            data = data['EPIC'];
            ...
            if (data.patientDemographics.dateOfBirth) {
              ...
              let currentYear = this.activityService.moment(new Date()).format('YY', 'en', true);

              if (currentYear < year) {
                splitted[splitted.length - 1] = '19' + year;
                date = splitted.join('/');
              }
              let dob = this.activityService.moment(new Date(date), 'MM/DD/YYYY').format('MM/DD/YYYY', 'en', true);
              this.age = this.activityService.moment(new Date()).diff(new Date(dob), 'years');
  ...
  updateActivity(field_last_updated_instant: string, newRegister: boolean) {
    if (field_last_updated_instant !== '') {
      this.activity[field_last_updated_instant] = this.activityService.moment().format('MM/DD/YYYY', 'en', true);
    }
    this.activity['pom_id'] = this.pomId;
    this.activityService.updateActivity(this.activity, this.encunterClosed, newRegister);
    this.updateTooltips();
  }
  ...
  updateTooltips() {
    this.msTypeTooltip = (this.activity.ms_type && this.activity.ms_type !== '-') ? `${this.activity.ms_type}\nEntered on: ${this.activity['ms_type_last_updated_instant']}` : '';
    this.ageOfOnsetTooltip = (this.activity.age_of_onset_last_updated_instant && this.activity.age_of_onset && this.activity.age_of_onset !== '-') ? `Entered on: ${this.activity.age_of_onset_last_updated_instant}` : '';
    this.msStatus1Tooltip = (this.activity.ms_status_1_last_updated_instant && this.activity.ms_status_1 && this.activity.ms_status_1 !== '-') ? `Entered on: ${this.activity.ms_status_1_last_updated_instant}` : '';
    this.msStatus2Tooltip = (this.activity.ms_status_2_last_updated_instant && this.activity.ms_status_2 && this.activity.ms_status_2 !== '-') ? `Entered on: ${this.activity.ms_status_2_last_updated_instant}` : '';
  }
}
